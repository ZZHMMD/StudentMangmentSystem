...
<input type="hidden" id="pagecount" value="${page.pagecount}">
	...
	<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">			
		...
		<div class="row">
			<div class="col-lg-12">
				<h1 class="page-header">学生档案管理</h1>
			</div>
		</div><!--/.row-->
		...
		<div >
			<div>
				<div class="col-xs-12 ">
					<form action="${pageContext.request.contextPath}/StudentServlet?op=find&currentPageIndex=1" id="findform" method="post">
						<div class="row">
							...
							<div class="col-sm-5" >
								<div class="form-group">
									<label class="lb-control">主修专业:</label>
									<div class="col-sm-9">
				...
				<div class="col-xs-12">
					...
					<div class="row">
						<div class="a-width">
							<div class="dataTables_info" id="sample-table-2_info">
								<div>
									<a class="btn btn-info" href="${pageContext.request.contextPath }/studentAdd.jsp"> 添加学籍
									</a>
								</div>
							</div>
						</div>				
                    ...
                    <c:choose>
                    <c:when test="${empty pageMajor.list}" >
                        <tr>
                    ...
                    <c:otherwise>
							<tbody>
								<c:forEach items="${pageMajor.list}" var="item" varStatus="status">
									<tr>
										<td ><label> <input type="checkbox"
												name="ids" id="ids" value="${item.id}" /> <span class="lbl"></span>
										</label></td>
										<td>${(pageMajor.currentPageIndex-1)*5+status.index +1 }</td>
										<td>${item.id }</td>
										<td>${item.name }</td>
										<td>${item.sex }</td>
										<td>${item.location }</td>
										...
										<td>
											<div>
												 <a href="${pageContext.request.contextPath }/StudentServlet?op=edit&id=${item.id}" > 编辑</a>
		...
		<table align="center">
        <tr>
        <td width="20%" align="left"><strong>第</strong><font color=red>${pageMajor.currentPageIndex }</font><strong>页/共</strong><font color=red>${pageMajor.pagecount }</font><strong>页</strong></td>
        <td width="60%">
            <a href="${pageContext.request.contextPath }/StudentServlet?op=findMajor&currentPageIndex=${pageMajor.currentPageIndex -1}" style="text-decoration:none;">|&lt;</a>
            <c:forEach begin="${startIndex}" end="${endIndex }" var="n">
                <a href="${pageContext.request.contextPath }/StudentServlet?op=findMajor&currentPageIndex=${n}">${pageMajor.currentPageIndex == n ?"<font color=red>":"<font>"}${n }</font></a>
            </c:forEach>
            <a href="${pageContext.request.contextPath }/StudentServlet?op=findMajor&currentPageIndex=${pageMajor.currentPageIndex +1}" style="text-decoration:none;">&gt;|</a>
        </td>
        <td width="10%">
            <input type="text" value="${pageMajor.currentPageIndex }" name="currentPageIndex" size="2" id="pageindex">&nbsp;<a href="javascript:aJump()">跳转</a>
        </td>
        <td width="10%">
            <select name="currentPageIndex" onchange="selectJump(this.value)">
            <c:forEach begin="1" end="${pageMajor.pagecount }" var="c">
                <option value="${c }" ${pageMajor.currentPageIndex==c?"selected":"" }>第${c }页</option>
            </c:forEach>
            </select>
        </td>
	...
	function delmore(){
		...
		confirm("是否删除被选中学生学籍?","删除学籍将无法恢复!",function (isConfirm) {
            if (isConfirm) {
            	...
            	//将数据传递到服务端进行删除
            	window.location = "${pageContext.request.contextPath }/StudentServlet?op=delmore&ids="+s+"&currentPageIndex=${pageMajor.currentPageIndex}";
	 ...
	 $(".delete_a").on('click',function(){
		     ...
		     var url="StudentServlet?op=delete&currentPageIndex=${pageMajor.currentPageIndex}&id="+id;
	...
	function aJump(index){
    	...
    	window.location.href="${pageContext.request.contextPath }/StudentServlet?op=findMajor&currentPageIndex="+index+"&currentPageIndex=${pageMajor.currentPageIndex}";
    }
