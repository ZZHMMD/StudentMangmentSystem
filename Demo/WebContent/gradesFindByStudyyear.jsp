...
<body>
<input type="hidden" id="pagecount" value="${pageStudyyearGrade.pagecount}">
	...
	<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">			
		...
		<div >
			<div>
						...
						<div class="row">
							...
							<div class="col-sm-5" >
								<div class="form-group">
									...
									<div class="col-sm-9">
										<input type="text" name="studyseason" id="studyseason"  placeholder="学期" class="form-control">
									</div>
				...
				<div class="col-xs-12">
                    ...
                    <c:choose>
                    <c:when test="${empty pageStudyyearGrade.list}" >
                        <tr>
                    ...
                    <c:otherwise>
							<tbody>
								<c:forEach items="${pageStudyyearGrade.list}" var="item" varStatus="status">
									<tr>
										<td ><label> <input type="checkbox"
												name="ids" id="ids" value="${item.id}" /> <span class="lbl"></span>
										</label></td>
										<td>${(pageStudyyearGrade.currentPageIndex-1)*5+status.index +1 }</td>
		...
		<table align="center">
        <tr>
        <td width="20%" align="left"><strong>第</strong><font color=red>${pageStudyyearGrade.currentPageIndex }</font><strong>页/共</strong><font color=red>${pageStudyyearGrade.pagecount }</font><strong>页</strong></td>
        <td width="60%">
            <a href="${pageContext.request.contextPath }/GradeServlet?op=findStudyyear&currentPageIndex=${pageStudyyearGrade.currentPageIndex -1}" style="text-decoration:none;">|&lt;</a>
            <c:forEach begin="${startIndex}" end="${endIndex }" var="n">
                <a href="${pageContext.request.contextPath }/GradeServlet?op=findStudyyear&currentPageIndex=${n}">${pageStudyyearGrade.currentPageIndex == n ?"<font color=red>":"<font>"}${n }</font></a>
            </c:forEach>
            <a href="${pageContext.request.contextPath }/GradeServlet?op=findStudyyear&currentPageIndex=${pageStudyyearGrade.currentPageIndex +1}" style="text-decoration:none;">&gt;|</a>
        </td>
        <td width="10%">
            <input type="text" value="${pageStudyyearGrade.currentPageIndex }" name="currentPageIndex" size="2" id="pageindex">&nbsp;<a href="javascript:aJump()">跳转</a>
        </td>
        <td width="10%">
            <select name="currentPageIndex" onchange="selectJump(this.value)">
            <c:forEach begin="1" end="${pageStudyyearGrade.pagecount }" var="c">
                <option value="${c }" ${pageStudyyearGrade.currentPageIndex==c?"selected":"" }>第${c }页</option>
            </c:forEach>
            </select>
        </td>
	...
	function delmore(){
		...
		confirm("是否删除被选中学生学籍?","删除学籍将无法恢复!",function (isConfirm) {
            if (isConfirm) {
            	...
            	//将数据传递到服务端进行删除
            	window.location = "${pageContext.request.contextPath }/GradeServlet?op=delmore&ids="+s+"&currentPageIndex=${pageStudyyearGrade.currentPageIndex}";
	 ...
	 $(".delete_a").on('click',function(){
		     ...
		     var url="GradeServlet?op=delete&currentPageIndex=${pageStudyyearGrade.currentPageIndex}&id="+id;
	...
	function aJump(index){
    	...
    	window.location.href="${pageContext.request.contextPath }/GradeServlet?op=findStudyyear&currentPageIndex="+index+"&currentPageIndex=${pageStudyyearGrade.currentPageIndex}";
    }
