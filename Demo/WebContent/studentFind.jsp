							...
							<tbody>
									<tr>
										<td ><label> <input type="checkbox"
												name="ids" id="ids" value="${student.id}" /> <span class="lbl"></span>
										</label></td>
										<td>1</td>
										<td>${student.id }</td>
										<td>${student.name }</td>
										...
										<td>
											<div>
												 <a href="${pageContext.request.contextPath }/StudentServlet?op=edit&id=${student.id}" > 编辑</a>
												 <a  class="delete_a" data-did="${student.id }" href="javascript:void(0);"> 删除</a>
											</div>
										</td>
									</tr>
							</tbody>
	...
	function delmore(){
		...
		confirm("是否删除被选中学生学籍?","删除学籍将无法恢复!",function (isConfirm) {
            if (isConfirm) {
            	...
            	//将数据传递到服务端进行删除
            	window.location = "${pageContext.request.contextPath }/StudentServlet?op=delmore&ids="+s+"&currentPageIndex=${page.currentPageIndex}";
	 ...
	 $(".delete_a").on('click',function(){
		     ...
		     var url="StudentServlet?op=delete&currentPageIndex=${page.currentPageIndex}&id="+id;
