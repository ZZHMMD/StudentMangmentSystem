...
<body>
<input type="hidden" id="pagecount" value="${pageXueji.pagecount}">
				...
				<ul class="user-menu">
					<li class="dropdown pull-right">
						...
						<ul class="dropdown-menu" role="menu">
							...
							<li><a   class="confirm"><span class="glyphicon glyphicon-log-out"></span> 注销</a></li>
						</ul>
					</li>
				</ul>
			</div>							
	...
	<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">			
		...
		<div>
			<div>
				...
				<div class="row">
                    ...
                    <c:choose>
                    <c:when test="${empty pageXueji.list}" >
                        <tr>
                    ...
                    <c:otherwise>
							<tbody>
								<c:forEach items="${pageXueji.list}" var="item" varStatus="status">
									<tr>
										<td ><label> <input type="checkbox"
												name="ids" id="ids" value="${item.studentid}" /> <span class="lbl"></span>
										</label></td>
										<td>${(pageXueji.currentPageIndex-1)*5+status.index +1 }</td>
		...
		<table align="center">
        <tr>
        <td width="20%" align="left"><strong>第</strong><font color=red>${pageXueji.currentPageIndex }</font><strong>页/共</strong><font color=red>${pageXueji.pagecount }</font><strong>页</strong></td>
        <td width="60%">
            <a href="${pageContext.request.contextPath }/XuejiServlet?op=page&currentPageIndex=${pageXueji.currentPageIndex -1}" style="text-decoration:none;">|&lt;</a>
            <c:forEach begin="${startIndex}" end="${endIndex }" var="n">
                <a href="${pageContext.request.contextPath }/XuejiServlet?op=page&currentPageIndex=${n}">${pageXueji.currentPageIndex == n ?"<font color=red>":"<font>"}${n }</font></a>
            </c:forEach>
            <a href="${pageContext.request.contextPath }/XuejiServlet?op=page&currentPageIndex=${pageXueji.currentPageIndex +1}" style="text-decoration:none;">&gt;|</a>
        </td>
        <td width="10%">
            <input type="text" value="${pageXueji.currentPageIndex }" name="currentPageIndex" size="2" id="pageindex">&nbsp;<a href="javascript:aJump()">跳转</a>
        </td>
        <td width="10%">
            <select name="currentPageIndex" onchange="selectJump(this.value)">
            <c:forEach begin="1" end="${pageXueji.pagecount }" var="c">
                <option value="${c }" ${pageXueji.currentPageIndex==c?"selected":"" }>第${c }页</option>
            </c:forEach>
            </select>
        </td>
	...
	<script>
	 ...
	 $(".delete_a").on('click',function(){
	     ...
	     var url="XuejiServlet?op=delete&currentPageIndex=${pageXueji.currentPageIndex}&id="+id;
...
function delmore(){
		...
		confirm("是否删除被选中学生学籍?","删除学籍将无法恢复!",function (isConfirm) {
            if (isConfirm) {
            	...
            	//将数据传递到服务端进行删除
            	window.location = "${pageContext.request.contextPath }/XuejiServlet?op=delmore&ids="+s+"&currentPageIndex=${pageXueji.currentPageIndex}";
...
function checkall(flag){
	...
	function aJump(){
    	...
    	window.location.href="${pageContext.request.contextPath }/XuejiServlet?op=page&currentPageIndex="+index+"&currentPageIndex=${pageXueji.currentPageIndex}";
    }
